...
: 2pick 2 * 1+ >r r@ pick r@ pick rdrop ;
: 2fdup ( d1 d2 -- d1 d2 d1 d2 )
	2over	\ d1 d2 d1
	2over	\ d1 d2 d1 d2
;
: 3fdup ( d1 d2 d3 -- d1 d2 d3 d1 d2 d3 )
	2 2pick  \ d1 d2 d3 d1
	2 2pick  \ d1 d2 d3 d1 d2
	2 2pick  \ d1 d2 d3 d1 d2 d3
;

180,0 2constant pi-deg
: atan2-quadrant-adjust ( dy dx atan -- deg )
 	\ + π if x < 0 and y >= 0
 	\ - π if x < 0 and y < 0
	2swap d0< 0= if \ dy atan
		\ x > 0
		2nip
	else
		2swap d0< if \ atan
...
: atan2 ( dy dx -- da )
 	\ The atan function calculates the arctangent (the inverse tangent function) of x.
 	\ atan2 calculates the arctangent of y / x if x > 0
 	\ (if x equals 0, atan2 returns π/2 if y is positive, -π/2 if y is negative, or 0 if y is 0.)
 	\ returns in degrees

 	2dup 0,0 d<> if \ dx <> 0
 		2fdup		\ dy dx dy dx
 		f/ atan	    \ returns in degrees
 		atan2-quadrant-adjust
 	else 			\ dx == 0
 		2drop 		\ drop dx
 		2dup d0< if \ dy < 0
 			2drop
 			pi/2 dnegate rad2deg
 		else
			d0= if 	\ dy == 0
				0,0
			else 	\ dy > 0
...
: normalize3 ( fx fy fz - nx ny nz )
	2dup 2>r 			\ fx fy fz r: fz
	2over 2>r 			\ fx fy fz r: fz fy
	2 2pick 2>r 		\ fx fy fz r: fz fy fx
	**2 2-rot		\ fz^2 fx fy
	**2 2-rot		\ fy^2 fz^2 fx
	**2 2-rot		\ fx^2 fy^2 fz^2
	d+ d+  				\ fx^2+fy^2+fz^2
	sqrt				\ mag
	2dup d0= if 2r> 2r> 2r> 2drop ." divide by zero " exit then
	2dup 2r> 2swap f/ 	\ mag x/mag
	2over 2r> 2swap f/  \ mag x/mag y/mag
	2rot 2r> 2swap f/	\ x/mag y/mag z/mag
;

: f+ d+ ;
: f- d- ;
: f+! ( f addr -- ) >r r@ 2@ d+ r> 2! ;

\ leading spaces to get 10 columns, 3dp truncated
: f10..3 ( f -- ) tuck dabs 0 <# #s 2drop [char] . hold< f# f# f# drop sign 0 0 #> dup 10 swap - spaces type  ;
: f..3 ( f -- ) tuck dabs 0 <# #s 2drop [char] . hold< f# f# f# drop sign 0 0 #> type space ;
\ round up to 3dp
: fr3 2147483 0 2over d0< if d- else d+ then ;
: f.r3 fr3 f..3 ;
\ round up to 2dp
